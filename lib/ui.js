...
import { defaultProjectName, ensureDevConfig, readDevConfig, writeDevConfig } from "./dev-config.js";
...
async function configureProject(project, root) {
  ...
  const defaultName = defaultProjectName(root);
  const nextConfig = {
    ...baseConfig,
    name: existing?.name ?? defaultName,
    type: project.type,
    root,
    host: existing?.host ?? `${defaultName}.kmc.localhost`
  };
