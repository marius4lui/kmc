  ...
  while (true) {
    ...
    if (action === "defaultGroup") {
      ...
      settings.defaultGroup = await prompt(() =>
        select({ message: "Default group", choices: groups.map((group) => ({ name: group.label, value: group.id, description: group.description })) })
      );
    }
...
export async function runGroupedPicker(config, settings) {
  ...
  return prompt(() =>
    select({
      message: groupId === "favorites" ? "Favorites" : GROUPS[groupId]?.label ?? groupId,
      pageSize: 12,
      choices: [
