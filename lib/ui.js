...
import { initScripts, runScript, validateScripts } from "./scripts-cli.js";
...
async function chooseScript(loaded) {
  if (loaded.scripts.size === 0) return null;
  return prompt(() =>
    select({
      message: "Run KMC Script",
      loop: false,
      pageSize: 12,
      choices: [
        ...[...loaded.scripts.values()].map((script) => ({
          name: script.id,
          value: script.id,
          description: script.description || script.workflow.name || "KMC workflow"
        })),
        { name: "Back", value: null }
      ]
    })
...
export async function scriptsMenu(root = process.cwd()) {
  while (true) {
    clearScreen();
    console.log(color.bold("KMC Scripts"));
    console.log(color.dim("Local YAML workflows from .kmc/scripts.yml"));
    console.log("");

    const configured = existsSync(path.join(root, ".kmc", "scripts.yml"));
    let loaded = null;
    let status = null;
    if (configured) {
      try {
        loaded = await loadWorkflows(root, { validateCwds: false });
        status = await trustStatus(loaded);
        console.log(`${color.dim("Workflows")}  ${loaded.scripts.size}`);
        console.log(`${color.dim("Trust")}      ${status.trusted ? color.green("trusted") : status.changed ? color.yellow("changed") : color.dim("not trusted")}`);
      } catch (error) {
        console.log(color.red("The scripts configuration is invalid."));
        console.log(color.dim(error.message.split("\n")[0]));
      }
      console.log("");
    } else {
      console.log(color.dim("No .kmc/scripts.yml found. Initialize KMC Scripts to get started."));
      console.log("");
    }
    ...
    const action = await prompt(() =>
      select({
        ...
        loop: false,
        choices: [
          ...(loaded?.scripts.size ? [{ name: "Run script", value: "run", description: "Choose and execute a workflow" }] : []),
          ...(configured ? [{ name: "Validate", value: "validate", description: "Check registry, workflows, paths, and shells" }] : []),
          { name: configured ? "Initialize missing files" : "Initialize", value: "init", description: "Create starter files without overwriting existing files" },
          ...(loaded && !status?.trusted ? [{ name: "Trust repository", value: "trust", description: "Trust the current workflow file fingerprint" }] : []),
          ...(status?.trusted ? [{ name: "Remove trust", value: "untrust", description: "Require confirmation before the next execution" }] : []),
          { name: "Back", value: "back" }
        ]
      })
    );
    ...
    if (action === "init") {
      clearScreen();
      await initScripts(root);
      await waitForEnter();
    }
    if (action === "validate") {
      clearScreen();
      await validateScripts(root, { setExitCode: false });
      await waitForEnter();
    }
    if (action === "trust") {
      await trustRepository(loaded);
      console.log(`\n${color.green("Trusted")} ${loaded.projectRoot}`);
      await waitForEnter();
    }
    if (action === "untrust") {
      await untrustRepository(root);
      console.log(`\n${color.yellow("Trust removed")} ${path.resolve(root)}`);
      await waitForEnter();
    }
    if (action === "run") {
      const id = await chooseScript(loaded);
      if (id) {
        clearScreen();
        await runScript(id, [], root);
        await waitForEnter();
      }
    }
  }
...
export async function mainMenu(config, settings) {
  ...
  return prompt(() =>
    select({
      ...
      choices: [
        { name: "Run", value: "run" },
        { name: "KMC Scripts", value: "scripts", description: "Run and manage local YAML workflows" },
