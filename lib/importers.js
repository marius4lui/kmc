...
import { GROUPS, SCHEMA_URL } from "./constants.js";
import { getGroups, groupShell, serializeCommand, serializeGroup, writeConfig } from "./store.js";
...
export async function importCommands(config, sourceIds, verbose = true) {
  ...
  for (const command of imports) {
    ...
    if (!importGroups.has(groupId)) {
      importGroups.set(groupId, { ...groupShell(groupId, true), ...GROUPS[groupId], commands: [] });
    }
    importGroups.get(groupId).commands.push(command);
  }
  const keptGroups = getGroups(config).filter((group) => !selected.has(group.id));
  const nextGroups = [...keptGroups, ...[...importGroups.values()].map(serializeGroup)];
  await writeConfig({ ...config, "$schema": SCHEMA_URL, groups: nextGroups });
  if (verbose) {
  ...
  return { ...config, "$schema": SCHEMA_URL, groups: nextGroups };
}
